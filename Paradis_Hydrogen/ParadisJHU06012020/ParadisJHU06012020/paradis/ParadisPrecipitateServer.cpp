...
bool ParadisPrecipitateServer::Set(Home_t *poHome) {
  Reset();
  if (strlen(poHome->param->PrecipitatesFileName) < 1) {
    return false;
  }
  // try to read the precipitates file
  if (!m_oStructure.Set(string(poHome->param->PrecipitatesFileName),
                        poHome->param->Dimensions[0],
                        poHome->param->Dimensions[1],
                        poHome->param->Dimensions[2])) {
    char cFilePath[256];
    sprintf(cFilePath, "../%s", poHome->param->PrecipitatesFileName);
...
void ParadisPrecipitateServer::CheckNodes(Home_t *poHome) const {
