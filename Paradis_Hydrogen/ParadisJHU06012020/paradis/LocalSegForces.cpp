...
void LocalSegForces(Home_t *home,
   ...
   */
  ofstream outFile;
  outFile.open("PK_Report_force.data");
  ...
  for (i = 0; i < nativeSegListCnt; i++) {
    ...
    poExternalLoadServer->GetSegmentForce(home, bx1, by1, bz1, x1, y1, z1, x2,y2, z2, f1, f2);

    outFile << node1->myTag.index <<" "<<node2->myTag.index <<" "<<  x1 << " " << y1 <<" " << z1 <<" "<< x2 << " " << y2 << " " << z2 <<" " << f1[0] << " " << f1[1]<< " " << f1[2] << " " << f2[0] << " " << f2[1] << " " << f2[2] << endl;
