...
void ParadisExternalLoadServer::GetSegmentForce(Home_t *poHome, double bx,
  ...
  //  FILE *fp4;
  std::string stress_filename = "Exstress"; 
  stress_filename.append(std::to_string(poHome->cycle)); 
  stress_filename.append(".txt"); 
//  fp4 = fopen(stress_filename.c_str(),"a");
  ...
  if (poHome->param->EnableFEM == 1) {
    ...
    strb[2] = poHome->param->appliedStress[4] * bx +
              poHome->param->appliedStress[3] * by +
              poHome->param->appliedStress[2] * bz;
    //fprintf(fp4, "%lf %lf %lf ",strb[0],strb[1],strb[2]);
  ...
  //fprintf(fp4, "%lf %lf %lf ",x1,y1,z1);
  //fprintf(fp4, "%lf %lf %lf ",x2,y2,z2);
  double xi[3];
  xi[0] = x2 - x1;
  xi[1] = y2 - y1;
  xi[2] = z2 - z1;
  //fprintf(fp4, "%lf %lf %lf ",xi[0],xi[1],xi[2]);
