...
void ReportForce(Home_t *home) {
  ofstream outFile1;
  ofstream outFile2;
  outFile1.open("Sigb_Loc.data");
  outFile2.open("Sigb_Rem.data");
  // int start_s5=clock();
  unsigned int i = 0;
  Node_t *poNode = NULL;
  for (i = 0; i < home->newNodeKeyPtr; i++) {
    ...
    outFile1 <<  poNode->x << " " << poNode->y <<" " << poNode->z <<" "<<poNode->sigbLoc[0]<< " " << poNode->sigbLoc[1] << " " << poNode->sigbLoc[2] << endl;

    outFile2 <<  poNode->x << " " << poNode->y <<" " << poNode->z <<" "<<poNode->sigbRem[0]<< " " << poNode->sigbRem[1] << " " << poNode->sigbRem[2] << endl;
